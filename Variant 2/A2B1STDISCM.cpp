...
mutex printMutex;
vector<vector<int>> primeNumbers;
...
void searchPrimeNumbers(int startRange, int endRange, int threadIndex) {
    vector<int> primes;
    ...
    primeNumbers[threadIndex] = primes;
}
...
void createThreads(int xNumThreads, int yEndRange) {
    ...
    for (int i = 0; i < xNumThreads; i++) {
        ...
        threads.emplace_back(searchPrimeNumbers, startRange, endRange, i);
...
int main() {
    ...
    primeNumbers.resize(xNumThreads);
